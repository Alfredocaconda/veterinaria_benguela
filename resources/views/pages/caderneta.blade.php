        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between">
                <div class="header-title" style="display: flex; justify-content: space-between; width: 100%">
                    <h4 class="card-title">Caderneta de Vacinação</h4>
                    ...
                    <tbody>
                        @foreach ($valor as $func)
                            <tr>
                                ...
                                <td>{{$func->animal->nome}}</td>
                                <td>{{$func->animal->raca}}</td>
                    ...
                    <form action="{{ route('cadern.store') }}" method="post">
                        ...
                        <div class="row">
                            <div class="form-group col-12 col-md-6 col-lg-6">
                                <label for="id_animal">Dados do Responsavel do Animal <span style="color: red;">*</span></label>
                                <select class="form-control" name="id_animal" id="id_animal">
                                    <option value="">Selecionar o Responsavel do Animal</option>
                                    @foreach (App\Models\Animal::with('proprietario')->orderBy('nome','ASC')->get() as $animal)
                                        <option value="{{ $animal->id }}">
                                            {{ $animal->proprietario->nome }} — {{ $animal->nome }}
                                        </option>
                                    @endforeach
                                </select>
                            </div> 
